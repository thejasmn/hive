...
public class DateWritable implements WritableComparable<DateWritable> {
  ...
  private static final LocalDate EPOCH_LOCALDATE = new LocalDate(1970, 1, 1);
  private static final DateTime EPOCH_DATETIME = new DateTime(1970, 1, 1, 0, 0, 0);
  ...
  public static long daysToMillis(int d) {
    return EPOCH_DATETIME.plusDays(d).getMillis();
  }

  public static int dateToDays(Date d) {
    // Get the year/month/day based on the local time zone,
    // then find difference between that and our epoch date.
    LocalDate datePortion = new LocalDate(d);
    return Days.daysBetween(EPOCH_LOCALDATE, datePortion).getDays();
  }
